...
from utils import Visualizer, k_fold, COLoss, write_results, get_raw_points, interpolation
from utils.evaluate_lane import LaneEval
import models
...
def test(**kwargs):
    ...
    for inputs, roots in tqdm(testloader, ncols=80, desc='testing: '):
        ...
        write_results(predictions, file, opt, roots)
...
def train(**kwargs):
    ...
    # 配置损失函数和优化器
    criterion = COLoss(opt.coeff, opt.bg_weight).to(opt.device)
